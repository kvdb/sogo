...
- (NSString *) _contentByRemovingDuplicatesFromString: (NSString *) iCalString
{
  ...
  /* Avoid parsing objects that cannot hold a duplicate: a repeat needs the
     marker to appear at least twice. Multiple VALARMs are legal, so their
     second marker only means the object is worth parsing, not that it is
     malformed. */
